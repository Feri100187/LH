...
let root = path.join(project, "release/web"), port = Number(process.env.PORT || 18765), probe = false, m01Probe = false, m1Probe = false;
for (let i = 2; i < process.argv.length; i++) {
    const key = process.argv[i];
    if (key === "--probe") { probe = true; continue; }
    if (key === "--m01-probe") { m01Probe = true; continue; }
    if (key === "--m1-probe") { m1Probe = true; continue; }
...
const m1File = path.join(project, "tools/testing/m1-runtime.js");
...
if (m1Probe && !fs.existsSync(m1File)) throw new Error("M1 probe script is missing");
...
const server = http.createServer((req, res) => {
    ...
    else if (m1Probe && pathname === "/__baseline__/m1-runtime.js") file = m1File;
    ...
    fs.stat(file, (error, stat) => {
        ...
        if ((probe || m01Probe || m1Probe) && file === path.join(root, "index.html")) {
            ...
            const scripts = (probe ? '<script defer src="/__baseline__/runtime-probe.js"></script>' : '')
                ...
                + (m1Probe ? '<script defer src="/__baseline__/m1-runtime.js"></script>' : '');
...
server.listen(port, "127.0.0.1", () => console.log(JSON.stringify({ url: `http://127.0.0.1:${port}`, root, probe, m01Probe, m1Probe }, null, 2)));
