...
let root = path.join(project, "release/web"), port = Number(process.env.PORT || 18765), probe = false;
...
const server = http.createServer((req, res) => {
    ...
    fs.stat(file, (error, stat) => {
        ...
        if (probe && file === path.join(root, "index.html")) {
            // Test instrumentation exists only in this response. Build files and normal game delivery stay unchanged.
            const html = fs.readFileSync(file, "utf8");
            res.end(html.replace(/<head([^>]*)>/i, '<head$1><script defer src="/__baseline__/runtime-probe.js"></script>'));
...
server.listen(port, "127.0.0.1", () => console.log(JSON.stringify({ url: `http://127.0.0.1:${port}`, root, probe }, null, 2)));
